...
public class HpoMapper {
  ...
  void transform(BufferedReader reader, BufferedWriter writer) {
    ...
    stack.addFirst(PREFIX + "HP_0000118"); // HP:0000118 Phenotypic abnormality

    while (!stack.isEmpty()) {
      String obj = stack.removeFirst();
      hpo.getGraphs()
          .forEach(
              graph -> {
                graph
                        ...
                        .forEach(
                            node -> {
                              if (node.getId().equals(hpoTermId)) {
                                ...
                                String description =
                                    meta != null && meta.getDefinition() != null
                                        ? meta.getDefinition().getVal()
                                        : null;
                                if (description != null) {
    ...
    StatefulBeanToCsv<HpoTerm> statefulBeanToCsv =
        new StatefulBeanToCsvBuilder<HpoTerm>(writer)
