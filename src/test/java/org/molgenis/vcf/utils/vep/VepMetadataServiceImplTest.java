...
class VepMetadataServiceImplTest {
  ...
  void load() throws FileNotFoundException {
    ...
    NestedField nestedField1 = NestedField.builder().id("VKGL_CL").index(0).numberType(NumberType.NUMBER)
        .numberCount(1).type(ValueType.STRING).fieldType(INFO_VEP).parent(parent).required(false).build();
    NestedField nestedField2 = NestedField.builder().id("CAPICE_CL").index(3).numberType(NumberType.NUMBER)
        .numberCount(1).type(ValueType.CATEGORICAL).categories(
            Set.of("P", "LP", "VUS", "LB", "B")).fieldType(INFO_VEP).parent(parent).required(false).build();
    NestedField nestedField3 = NestedField.builder().id("PHENO").index(1).separator('&').numberType(NumberType.OTHER)
        .type(ValueType.INTEGER).fieldType(INFO_VEP).parent(parent).required(false).build();
    NestedField nestedField4 = NestedField.builder().id("VIPP").index(2).separator('&').numberType(NumberType.OTHER)
        .type(ValueType.STRING).fieldType(INFO_VEP).parent(parent).required(false).build();
    Map<String, NestedField> vepMeta = new HashMap<>();
    vepMeta.put("VKGL_CL", nestedField1);
    vepMeta.put("CAPICE_CL", nestedField2);
    vepMeta.put("PHENO", nestedField3);
    vepMeta.put("VIPP", nestedField4);
